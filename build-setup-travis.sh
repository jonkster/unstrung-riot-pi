...
mkdir -p ${BUILDTOP}/${HOST}
    ...
    if $build64 && [ ! -d "${BUILDTOP}/x86_64/mbedtls" ]; then
        (cd ${BUILDTOP} && rm -rf x86_64/mbedtls && mkdir -p x86_64/mbedtls && cd x86_64/mbedtls && cmake -DCMAKE_INSTALL_PREFIX=$BUILDTOP/x86_64 ../../mbedtls && make CFLAGS='--coverage -g3 -O0' && make install)
    fi

    if $build32 && [ ! -d "${BUILDTOP}/i386/mbedtls" ]; then
        (cd ${BUILDTOP} && rm -rf i386/mbedtls && mkdir -p i386/mbedtls && cd i386/mbedtls && cmake  -DCMAKE_INSTALL_PREFIX=$BUILDTOP/i386 ../../mbedtls && make CFLAGS='--coverage -g3 -O0' && make install)
    fi

LIBPCAP=${BUILDTOP}/host/libpcap/libpcap.a
...
then
    ...
    if $build64 && [ ! -d "${BUILDTOP}/x86_64/tcpdump-4.8.1/." ]; then
        (cd ${BUILDTOP} && mkdir -p x86_64/libpcap-1.8.1 && cd x86_64/libpcap-1.8.1 && ../../libpcap/configure --prefix=$HOME/stuff --target=x86_64-linux-gnu && make  CFLAGS="-fPIC")
        (cd ${BUILDTOP} && ln -f -s x86_64/libpcap-1.8.1 libpcap && mkdir -p x86_64/tcpdump-4.8.1 && cd x86_64/tcpdump-4.8.1 && ../../tcpdump/configure --prefix=$HOME/stuff --target=x86_64-linux-gnu && make  CFLAGS=" -fPIC")
        (cd ${BUILDTOP}/x86_64 && ln -s -f tcpdump-4.8.1 tcpdump )
    fi

    if $build32 && [ ! -d "${BUILDTOP}/i386/tcpdump-4.8.1/." ]; then
        (cd ${BUILDTOP} && mkdir -p i386/libpcap-1.8.1 && cd i386/libpcap-1.8.1 &&  ../../libpcap/configure --prefix=$HOME/stuff --target=i686-pc-linux-gnu && make  CFLAGS="-fPIC")
        (cd ${BUILDTOP} && ln -f -s i386/libpcap-1.8.1 libpcap && mkdir -p i386/tcpdump-4.8.1 && cd i386/tcpdump-4.8.1 &&  ../../tcpdump/configure --prefix=$HOME/stuff --target=i686-pc-linux-gnu && make  CFLAGS="-fPIC")
        (cd ${BUILDTOP}/i386 && ln -s -f tcpdump-4.8.1 tcpdump )
    fi

    (cd $BUILDTOP/${HOST}/libpcap-1.8.1 && sudo make install)
fi
...
then
    ...
    (cd ${BUILDTOP} && mkdir -p ${HOST}/libcbor && cd ${HOST}/libcbor && cmake ../../libcbor -DCMAKE_INSTALL_PREFIX:PATH=${BUILDTOP} && make && sudo make install)

    if $build64 && [ ! -d "${BUILDTOP}/x86_64/libcbor" ]; then
        (cd ${BUILDTOP} && mkdir -p x86_64/libcbor && cd x86_64/libcbor && cmake ../../libcbor -DCMAKE_C_FLAGS:STRING="-fPIC" -DCMAKE_INSTALL_PREFIX:PATH=${BUILDTOP}/x86_64 && make && sudo make install)
    fi

    if $build32 && [ ! -d "${BUILDTOP}/i386/libcbor" ]; then
        (cd ${BUILDTOP} && mkdir -p i386/libcbor && cd i386/libcbor && cmake ../../libcbor -DCMAKE_C_FLAGS:STRING="-fPIC" -DCMAKE_INSTALL_PREFIX:PATH=${BUILDTOP}/i386 && make && sudo make install)
    fi
fi
